...
divide :: [a] -> [([a], [a])]
divide xs = [splitAt n xs | n <- [1..(length xs - 1)]]

buildExpression :: ([Exp], [Exp]) -> [Exp]
buildExpression (es1, es2) =
...
toExpression xs =
  concat [buildExpression (toExpression l, toExpression r) | (l, r) <- divide xs]

generate :: [Double] -> [Exp]
generate ns = concatMap toExpression (permutations ns)

solve24 :: [Double] -> [String]
solve24 ns = [showExp x | x <- generate ns, eval x == 24.0]
